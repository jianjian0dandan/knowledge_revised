...
reload(sys)
sys.path.append('../../')
from global_config import ZMQ_VENT_PORT_FLOW2, ZMQ_CTRL_VENT_PORT_FLOW2, ZMQ_VENT_HOST_FLOW1, ZMQ_CTRL_HOST_FLOW1, BIN_FILE_PATH, WRITTEN_TXT_PATH
...
def send_weibo(sender, poller, controller, total_count=0, total_cost=0):
    ...
    #file_list = set(os.listdir(REPLICA_BIN_FILE_PATH))
    ...
    for each in file_list:
        ...
        if 'data' in each:
            ...
            print filename
            if '%s_yes2.txt' % filename not in txt_list and "20160308" in filename:
                ...
                fw = open(os.path.join(WRITTEN_TXT_PATH, '%s_yes2.txt' % filename), 'w')
