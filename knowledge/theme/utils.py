...
def create_theme_relation(node_key1, node1_list, node1_index_name, rel, node_key2, node2_id, node2_index_name, submit_user):
    node2_id_pinyin = p.get_pinyin(node2_id.decode('utf-8'))
    ...
    print eid_string
    es_event.update(index=special_event_name,doc_type=special_event_type,id=node2_id_pinyin,body={'doc':{'event':eid_string}})
    flag = create_rel(node_key1, node1_list, node1_index_name, rel, node_key2, node2_id, node2_index_name, submit_user)
    return flag
...
def create_node_and_rel(node_key1, node1_list, node1_index_name,\
                        rel, node_key2, node2_id, node2_index_name, submit_user):
    ...
    if len(node_l)>0:#判断对否有该节点存在
        ...
        # theme_dict['k_label'] = 'admin@qq.com_test&admin@qq.com_test2@admin_test3'
        topic_id = p.get_pinyin(node2_id.decode('utf-8'))
        ...
        new_theme = Node(special_event_node, event=node2_id)
...
def query_detail_theme(theme_name, submit_user):
    topic_id = p.get_pinyin(theme_name.decode('utf-8'))
